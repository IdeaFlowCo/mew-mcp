...
export class NodeService extends AuthService {
    ...
    async findNodeByText({ parentNodeId, nodeText, }) {
        ...
        return childNodes.find((node) => node &&
            node.content &&
    ...
    async getLayerData(objectIds) {
        return this.requestQueue.enqueue(async () => {
            ...
            if (!response.ok) {
                throw new NodeOperationError(`Failed to fetch layer data: ${response.statusText}`, objectIds[0], response.status, await response.text());
            }
    ...
    async updateNode(nodeId, updates) {
        ...
        try {
            ...
            if (!existingNode) {
                throw new NodeOperationError(`Node with ID ${nodeId} not found.`, nodeId);
            }
            const updatePayload = {
            ...
            await this.requestQueue.enqueue(async () => {
                ...
                if (!response.ok) {
                    ...
                    throw new NodeOperationError(`Failed to update node ${nodeId}: ${response.statusText}`, nodeId, response.status, responseText);
                }
            });
        }
        catch (error) {
    ...
    async deleteNode(nodeId) {
        ...
        const existingNode = layerData.data.nodesById[nodeId];
        if (!existingNode) {
            console.warn(`[NodeService] Node with ID ${nodeId} not found for deletion. Skipping.`);
            return;
        }
        const deletePayload = {
        ...
        await this.requestQueue.enqueue(async () => {
            ...
            if (!response.ok) {
                const responseText = await response.text();
                throw new NodeOperationError(`Failed to delete node ${nodeId}: ${response.statusText}`, nodeId, response.status, responseText);
            }
        });
    }
    /**
    ...
    async addNode(input) {
        ...
        if (content.type === NodeContentType.Replacement &&
        ...
        console.log("[NodeService] addNode /sync payload:", JSON.stringify(payload, null, 2));
        await this.requestQueue.enqueue(async () => {
            const response = await fetch(`${this.config.baseUrl}/sync`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                const responseText = await response.text();
                throw new NodeOperationError(`Failed to add node: ${response.statusText}`, newNodeId, response.status, responseText);
            }
        });
        return {
            newNodeId,
            ...
            parentChildRelationId,
            referenceNodeId: content.type === NodeContentType.Replacement &&
                content.replacementNodeData
                ? content.replacementNodeData.referenceNodeId
                : "",
            referenceCanonicalRelationId: content.type === NodeContentType.Replacement &&
                content.replacementNodeData
    ...
    getNodeUrl(nodeId) {
        if (!this.currentUserId) {
            console.warn("[NodeService] getNodeUrl called before currentUserId is set. URL might be incorrect.");
    ...
    static parseUserRootNodeIdFromUrl(url) {
        ...
        if (!regex.test(url)) {
            console.error("[NodeService] Invalid user root node URL format for parsing:", url);
        ...
        try {
            ...
            console.error("[NodeService] Error decoding URL part:", lastPart, e);
    ...
    getUserRootNodeId() {
        if (!this.currentUserId) {
            throw new NodeOperationError("Current User ID is not set. Cannot determine root node ID.", "unknown", // No specific node ID applies here
            500, "User ID not available");
        }
        if (!this.config.baseNodeUrl) {
            throw new NodeOperationError("Base Node URL is not configured. Cannot determine root node ID.", "unknown", 500, "Base Node URL not available");
        }
        ...
        console.error("[NodeService] Constructed userRootNodeUrl:", userRootNodeUrl); // For debugging
        try {
            return NodeService.parseUserRootNodeIdFromUrl(userRootNodeUrl);
        }
        catch (error) {
            console.error("[NodeService] Failed to parse user root node ID from URL:", userRootNodeUrl, error);
            // Re-throw as a NodeOperationError or a more specific error type if desired.
            throw new NodeOperationError(`Failed to parse user root node ID from URL: ${error instanceof Error ? error.message : String(error)}`, "unknown", 500, `URL: ${userRootNodeUrl}`);
        }
    }
}
