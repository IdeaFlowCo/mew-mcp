...
console.error(`[Mew MCP] [mcp.ts] Script execution started. Version ID: ${SCRIPT_VERSION_ID}`); // DEBUG
...
console.error(`[Mew MCP] [mcp.ts] CURRENT_USER_ID: ${process.env.CURRENT_USER_ID || "NOT SET"}`);
console.error(`[Mew MCP] [mcp.ts] BASE_URL: ${process.env.BASE_URL || "NOT SET"}`);
console.error(`[Mew MCP] [mcp.ts] BASE_NODE_URL: ${process.env.BASE_NODE_URL || "NOT SET"}`);
console.error(`[Mew MCP] [mcp.ts] AUTH0_DOMAIN: ${process.env.AUTH0_DOMAIN ? "SET" : "NOT SET"}`);
console.error(`[Mew MCP] [mcp.ts] AUTH0_CLIENT_ID: ${process.env.AUTH0_CLIENT_ID ? "SET" : "NOT SET"}`);
console.error(`[Mew MCP] [mcp.ts] AUTH0_CLIENT_SECRET: ${process.env.AUTH0_CLIENT_SECRET ? "SET (sensitive)" : "NOT SET"}`);
console.error(`[Mew MCP] [mcp.ts] AUTH0_AUDIENCE: ${process.env.AUTH0_AUDIENCE ? "SET" : "NOT SET"}`);
...
function main() {
    ...
    try {
        console.error("[Mew MCP] [mcp.ts] main() function: Calling startMCP..."); // DEBUG
        startMCP({
            port: port,
            // configPath is no longer passed
        });
        console.error(`[Mew MCP] Server init function called, listening on port ${port}`);
        console.error("[Mew MCP] [mcp.ts] main() function: startMCP call completed."); // DEBUG
    }
    catch (error) {
        // Added type annotation for error
        console.error("[Mew MCP] CRITICAL: Failed to start MCP server in main():", error);
...
try {
    main();
    console.error("[Mew MCP] [mcp.ts] main() call completed successfully (synchronous part)."); // DEBUG
}
catch (e) {
    console.error("[Mew MCP] [mcp.ts] CRITICAL SYNCHRONOUS ERROR IN MAIN EXECUTION:", e);
...
process.on("uncaughtException", (error) => {
    ...
    const stack = error && typeof error === "object" && "stack" in error
        ? String(error.stack)
        : "N/A";
    console.error(`[Mew MCP] [mcp.ts] Uncaught Exception Details: Name: ${name}, Message: ${message}`);
...
process.on("unhandledRejection", (reason, promise) => {
    ...
    if (reason instanceof Error) {
        console.error("[Mew MCP] [mcp.ts] Unhandled Rejection Reason Stack:", reason.stack);
    }
    else {
        console.error("[Mew MCP] [mcp.ts] Unhandled Rejection Reason (not an Error instance):", String(reason));
    }
    process.exit(1);
});
