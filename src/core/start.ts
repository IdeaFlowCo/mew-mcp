...
export function startMCP({ port }: StartMCPParams): void {
    ...
    app.post(
        ...
        asyncHandler(async (req: Request, res: Response) => {
            ...
            try {
                console.error(
                    "[Mew MCP] [core/start] /initialize called. Full Request body:", // More specific log
                    JSON.stringify(req.body, null, 2) // Stringify for better visibility
                );
                const requestId =
                    req.body && req.body.id !== undefined ? req.body.id : null; // Safer access to id
                console.error(
                    "[Mew MCP] [core/start] /initialize extracted request ID:",
                    requestId
                );
                ...
                res.json(response);
                console.error(
                    "[Mew MCP] [core/start] /initialize handler: Successfully processed and MCP compliant response sent."
                ); // Added for debugging
            } catch (error: any) {
                console.error(
